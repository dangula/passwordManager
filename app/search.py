...
def reader(queue, loops,KeyList,dbHandle,userName,searchKey,searchString,RESULT_LIST):
...
def doSearch(store_db,userName,passwd,searchKey,searchString):
    ...
    if not isinstance(user, User):
        ...
        threads = []
        for i in nfuncs:
            t = My_thread(funcs[i], (q, nloops,KeyList,dbHandle,userName,searchKey,searchString,RESULT_LIST), funcs[i].__name__)
            threads.append(t)
